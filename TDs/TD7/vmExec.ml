...
let next_state state =
  ...
  | (r, ((VmBytecode.VMI_Mkclos body_code) :: c), s) ->
      ...
      { VmBytecode.register = VmBytecode.VMV_closure (body_code, r) ;
        VmBytecode.code = c ;
        VmBytecode.stack = s }
  | (r, (VmBytecode.VMI_Apply :: c),
     ((VmBytecode.VMV_closure
         (body_code, (VmBytecode.VMV_env r0))) :: s)) ->
      (* Application. *)
      { VmBytecode.register = VmBytecode.VMV_env (r :: r0) ;
        VmBytecode.code = body_code ;
        VmBytecode.stack = (VmBytecode.VMV_code_addr c) :: s }
  | ((VmBytecode.VMV_env (h :: _)), ((VmBytecode.VMI_Access 0) :: c),
     s) ->
       (* Identifier : access here. *)
       { VmBytecode.register = h ;
         VmBytecode.code = c ;
         VmBytecode.stack = s }
  | ((VmBytecode.VMV_env (_ :: q)), ((VmBytecode.VMI_Access n) :: c),
     s) ->
       (* Identifier : access further (recursive case). *)
       { VmBytecode.register = VmBytecode.VMV_env q ;
         VmBytecode.code = (VmBytecode.VMI_Access (n - 1)) :: c ;
         VmBytecode.stack = s }
